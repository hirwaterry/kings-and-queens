...
const TIERS = [
  { icon:"🌱", label:"Novice",   minXP:0,    nextXP:150,    color:"text-white/50",   bg:"bg-white/5",       border:"border-white/10",      desc:"Just getting started. Show up!" },
  { icon:"🗡️", label:"Warrior",  minXP:150,  nextXP:500,    color:"text-blue-300",   bg:"bg-blue-500/10",   border:"border-blue-500/20",   desc:"You've been around a few weeks." },
  ...
  { icon:"🛡️", label:"Elite",    minXP:1200, nextXP:2500,   color:"text-amber-500",  bg:"bg-amber-700/15",  border:"border-amber-700/30",  desc:"Consistent contributor. Top 20%." },
  { icon:"⚔️", label:"Legend",   minXP:2500, nextXP:5000,   color:"text-slate-300",  bg:"bg-slate-400/15",  border:"border-slate-400/30",  desc:"You've earned serious respect." },
  { icon:"👑", label:"Champion", minXP:5000, nextXP:Infinity,color:"text-yellow-300", bg:"bg-yellow-400/20", border:"border-yellow-400/40", desc:"The realm's highest honour." },
];
...
const XP_EVENTS = [
  ...
  { label:"Streak multiplier (per week)", xp:"+10%", icon:"🔥", color:"text-orange-300"    },
];
...
export default function XPPage() {
  ...
  const nextIdx = rank ? TIERS.findIndex(t => t.label === rank.label) + 1 : 0;
      ...
      <div className="container mx-auto px-4 pt-8 pb-16 max-w-lg">
        ...
        {profile && rank && (
          <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.1 }}
            ...
            style={{ boxShadow:"0 0 40px rgba(250,204,21,0.1)" }}>
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                ...
                <p className="text-white/50 text-xs font-mono">{profile.display_name} · {profile.total_xp.toLocaleString()} XP</p>
              </div>
            ...
            {rank.nextXP !== Infinity ? (
              <>
                <div className="flex justify-between text-[10px] text-white/30 font-mono mb-1.5">
                  ...
                  <span>{profile.total_xp} / {rank.nextXP} XP → {TIERS[nextIdx]?.label}</span>
                </div>
                ...
                <p className="text-[10px] text-white/25 font-mono mt-1.5">
                  {rank.nextXP - profile.total_xp} XP to {TIERS[nextIdx]?.label}
                </p>
              </>
            ) : (
              <div className="text-center py-2">
                <p className="text-yellow-300 font-black text-sm">👑 Maximum rank achieved!</p>
              </div>
            )}
              ...
              return (
                <motion.div key={t.label} initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }}
                  ...
                  className={`flex items-center gap-4 p-4 rounded-2xl border transition-all
                              ${isCurrent ? `${t.bg} ${t.border} ring-1 ring-yellow-400/20` : `bg-white/4 border-white/8`}`}
                  style={isCurrent ? { boxShadow:"0 0 20px rgba(250,204,21,0.08)" } : {}}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {isCurrent && <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-yellow-400/20 text-yellow-300 font-mono">YOU</span>}
                    </div>
